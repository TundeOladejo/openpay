...
<template>
  <aside class="menu">
    <div
      class="is-flex is-3 is-justify-content-space-between navbar-brand px-5"
    >
    ...
    >
      <div class="navigation-items">
        <ul class="menu-list">
          ...
          >
            <a
              class="is-flex is-flex-wrap-wrap is-justify-content-space-between py-4"
            >
              <span
                ><i class="bi pr-3" :class="`bi-` + item.icon"></i
                >{{ item.name }}</span
              >
              <span
                v-if="item.notification != null"
                class="tag is-rounded ml-3"
                style="background-color: #0177fb; color: white"
